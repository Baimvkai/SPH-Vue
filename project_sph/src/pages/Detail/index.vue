    ...
    methods: {
      ...
      async addShopcar(){
        try {
          ...
          this.arrt = []
          // 获取产品参数
          this.spuSaleAttrList.map((v)=>{
            v.spuSaleAttrValueList.forEach(item=>{
              if(item.isChecked==1) {
                this.arrt.push(item.saleAttrValueName)
              }
            })
          })
          // console.log(this.arrt);
          sessionStorage.setItem('attr',JSON.stringify(this.arrt))
          this.$router.push({
            name:'AddCartSuccess',
